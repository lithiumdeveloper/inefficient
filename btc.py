...
def Gen():
    ...
    while True:
        ...
        # Generate the public address and WIF (assume these functions are defined elsewhere)
        address = getPublicKey(randomBytes)  
        key = getWif(randomBytes)  
        
        print(f"Generated Address: {address}")
        
        # Query the BlockCypher API for the balance
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                data = response.json()
                balance_satoshis = data.get("final_balance")  # Balance in satoshis
                balance_btc = balance_satoshis / 1e8  # Convert to BTC
                ...
                if balance_btc > 0.00: #testing CHANGE THIS LATER
                    ...
                    files.amendfile("lottery.txt", f"\n{address} : {key}")

            else:
                failedattempts += 1
                print(f"Error {response.status_code} : {response.reason}")
                time.sleep(60*(failedattempts*2))  # assuming it's a rate limit error
        except Exception as e:
            print(f"An error occurred: {e}")
        
        # Wait before generating the next address
        time.sleep(3)
